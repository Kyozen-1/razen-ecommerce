 ...
 * ==========================================================
 *
 * AJAX functions. This file must be executed only via AJAX. © 2017-2023 board.support. All rights reserved.
 *
 */

if (file_exists('../config.php')) require_once('../config.php');
if (defined('SB_CROSS_DOMAIN') && SB_CROSS_DOMAIN) header('Access-Control-Allow-Origin: *');
if (!isset($_POST['function'])) die('true');
...
if (sb_is_cloud()) {
    if (sb_cloud_ajax_function_forbidden($_POST['function'])) die('cloud_function_forbidden');
    sb_cloud_load();
}
...
function sb_ajax_execute() {
    switch ($_POST['function']) {
        ...
        case 'get-conversations':
            return sb_json_response(sb_get_conversations(sb_post('pagination'), sb_post('status_code'), sb_post('department'), sb_post('source')));
        case 'get-new-conversations':
            return sb_json_response(sb_get_new_conversations($_POST['datetime']));
        ...
        case 'new-conversation':
            return sb_json_response(sb_new_conversation($_POST['user_id'], sb_post('status_code'), sb_post('title', ''), sb_post('department', -1), sb_post('agent_id', -1), sb_post('source'), sb_post('extra')));
        ...
        case 'get-new-user-conversations':
            return sb_json_response(sb_get_new_user_conversations($_POST['user_id'], $_POST['datetime']));
        ...
        case 'send-bot-message': // Deprecated
            return sb_json_response(sb_dialogflow_message($_POST['conversation_id'], $_POST['message'], sb_post('token', -1), sb_post('dialogflow_language', sb_post('language')), sb_post('attachments', []), sb_post('event')));
        case 'update-message':
        ...
        case 'update-messages-status':
            return sb_json_response(sb_update_messages_status($_POST['conversation_id'], $_POST['message_ids'], sb_post('user_id')));
        case 'csv-users':
            return sb_json_response(sb_csv_users(sb_post('users_id')));
        case 'csv-conversations':
            return sb_json_response(sb_csv_conversations(sb_post('conversation_id', -1)));
        case 'transcript':
        ...
        case 'dialogflow-message':
            return sb_json_response(sb_dialogflow_message(sb_post('conversation_id'), $_POST['message'], sb_post('token', -1), sb_post('dialogflow_language', sb_post('language')), sb_post('attachments', []), sb_post('event'), sb_post('parameters'), sb_post('project_id')));
        case 'dialogflow-create-intent':
            return sb_json_response(sb_dialogflow_create_intent($_POST['expressions'], $_POST['response'], sb_post('agent_language', ''), sb_post('conversation_id')));
        case 'dialogflow-update-intent':
            return sb_json_response(sb_dialogflow_update_intent($_POST['intent_name'], $_POST['expressions'], sb_post('agent_language', '')));
        ...
        case 'dialogflow-intent': // Deprecated
            return sb_json_response(sb_dialogflow_create_intent($_POST['expressions'], $_POST['response'], sb_post('agent_language', '')));
        ...
        case 'dialogflow-smart-reply':
            return sb_json_response(sb_dialogflow_smart_reply($_POST['message'], sb_post('smart_reply_data'), sb_post('dialogflow_language'), sb_post('token'), sb_post('language_detection')));
        case 'dialogflow-smart-reply-update':
            return sb_json_response(sb_dialogflow_smart_reply_update($_POST['message'], $_POST['smart_reply_data'], sb_post('dialogflow_language'), sb_post('token'), sb_post('user_type', 'agent')));
        case 'dialogflow-smart-reply-generate-conversations-data':
            return sb_json_response(sb_dialogflow_smart_reply_generate_conversations_data());
        ...
        case 'path':
            return sb_json_response(sb_('The Support Board path is:') . '<br>' . SB_PATH);
        ...
        case 'aecommerce-sync':
            return sb_json_response(sb_aecommerce_sync($_POST['function'] == 'aecommerce-sync-admins' ? 'admin' : ($_POST['function'] == 'aecommerce-sync-sellers' ? 'seller' : 'customer')));
        ...
        case 'messenger-send-message':
            return sb_json_response(sb_messenger_send_message($_POST['psid'], $_POST['facebook_page_id'], sb_post('message', ''), sb_post('attachments', []), sb_post('metadata', [])));
        ...
        case 'whatsapp-send-template':
            return sb_json_response(sb_whatsapp_send_template($_POST['phone'], sb_post('language', ''), sb_post('conversation_url_parameter', ''), sb_post('user_name', ''), sb_post('user_email', ''), sb_post('template_name'), sb_post('phone_id')));
        ...
        case 'telegram-send-message':
            return sb_json_response(sb_telegram_send_message($_POST['chat_id'], sb_post('message', ''), sb_post('attachments', [])));
        case 'telegram-synchronization':
            return sb_json_response(sb_telegram_synchronization($_POST['token'], sb_post('cloud_token')));
        ...
        case 'direct-message':
            return sb_json_response(sb_direct_message($_POST['user_ids'], $_POST['message'], sb_post('subject')));
        ...
        case 'google-translate':
            return sb_json_response(sb_google_translate($_POST['strings'], $_POST['language_code']));
        ...
        case 'check-conversations-assignment':
            return sb_json_response(sb_check_conversations_assignment($_POST['conversations_ids'], sb_post('agent_id'), sb_post('department')));
        ...
        case 'martfury-sync-sellers':
            return sb_json_response(sb_martfury_import_users($_POST['function'] == 'martfury-sync-sellers'));
        case 'open-ai-message':
            return sb_json_response(sb_open_ai_message($_POST['message']));
        ...
        default:
            return '["error", "Support Board Error [ajax.php]: No functions found with name: ' . $_POST['function'] . '."]';
    }
}

function sb_json_response($result) {
    if (sb_is_error($result)) {
        return defined('SB_API') ? sb_api_error($result) : json_encode(['error', $result->code(), $result->function_name(), $result->message()], JSON_INVALID_UTF8_IGNORE);
    } else {
