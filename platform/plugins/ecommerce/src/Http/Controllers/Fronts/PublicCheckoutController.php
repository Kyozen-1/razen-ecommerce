...
class PublicCheckoutController
{
    ...
    public function getCheckout(
        ...
        if(isset($_GET['resultCode']))
        {
            if($_GET['resultCode'] == '00')
            {
                ...
                $token = strtok($uriArray[2], '?');

                $get_order = Order::where('token', $token)->first();
                $get_payment = Payment::where('order_id', $get_order->id)->first();
                $update_payment = Payment::find($get_payment->id);
                $update_payment->status = 'completed';
                $update_payment->save();

                return redirect(route('public.checkout.success', $token));
            }

            if($_GET['resultCode'] == '01')
            {
    ...
    public function getPaymentMethod()
    {
        ...
        $url = env('DUITKU_PAYMENT_METHOD_DEVELOPMENT');
