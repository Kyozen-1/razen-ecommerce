    ...
    public function boot(): void
    {
        ...
        if (function_exists('add_shortcode')) {
            ...
            add_filter(FILTER_ECOMMERCE_PROCESS_PAYMENT, function (array $data, Request $request) {
                ...
                // dd($data);
                // dd($request->all());
                // dd($request->address['name']);
                $paymentData = apply_filters(PAYMENT_FILTER_PAYMENT_DATA, [], $request);
                // dd($paymentData['orders']);

                // dd($paymentData['return_url']);
                // dd($paymentData);
                ...
                switch ($request->input('payment_method')) {
                    ...
                    case PaymentMethodEnum::BANK_TRANSFER:
                        // dd($paymentData);
                        // dd([$paymentData['callback_url'],$paymentData['return_url']]);
                        ...
                        $paymentMethod = $request->paymentMethod; // VC = Credit Card
                        $merchantOrderId = time() . ''; // dari merchant, unik
                        $productDetails = $request->name;
                        if($data['status_digital'] == 'tidak')
                        {
                            $email = $request->address['email']; // email pelanggan anda
                            $phoneNumber = $request->address['phone']; // nomor telepon pelanggan anda (opsional)
                            $customerVaName = $request->address['name']; // tampilan nama pada tampilan konfirmasi bank
                            // Customer Detail
                            $firstName = $request->address['name'];
                            $lastName = $request->address['name'];
                            $alamat = $request->address['address'];
                            $city = $request->address['city'];
                            $countryCode = $request->address['country'];
                        }
                        if($data['status_digital'] == 'ya')
                        {
                            $email = $data['email']; // email pelanggan anda
                            $phoneNumber = $data['phone']; // email pelanggan anda
                            $customerVaName = $data['name']; // email pelanggan anda
                            // Customer Detail
                            $firstName = $data['name'];
                            $lastName = $data['name'];
                            $alamat = $data['address'];
                        ...
                        $callbackUrl = $paymentData['callback_url']; // url untuk callback
                        $returnUrl = $paymentData['return_url']; // url untuk redirect
                        ...
                        foreach($paymentData['orders'] as $order)
                        {
                            foreach($order->products as $order_product)
                            {
                                $itemProduct = [
                                    ...
                                    'quantity' => (int) $order_product->qty,
                                    'name' => $order_product->product_name,
                                ];
                            }
                            $itemDetails[] = $itemProduct;
                        }



                        if($request->paymentMethod == 'OL' || $request->paymentMethod == 'SL')
                        {
                        ...
                        //echo $params_string;
                        $url = env('DUITKU_HTTP_TRANSAKSI_DEVELOPMENT'); // Sandbox
                        ...
                        //execute post
                        $request_ch = curl_exec($ch);
                        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                        $result_request = '';
                        if($httpCode == 200)
                        {
                            $result_ch = json_decode($request_ch, true);
                            $result_request = $result_ch;
                            // dd($result);
                        ...
                        {
                            $request_ch = json_decode($request_ch);
                            $error_message = "Server Error " . $httpCode ." ". $request_ch->Message;
                            $result_request = [
                                'statusMessage' => 'error',
                        ...
                        $data['result_transaksi_duitku'] = $result_request;
