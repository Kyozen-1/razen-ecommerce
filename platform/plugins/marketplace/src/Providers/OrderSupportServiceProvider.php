...
use Throwable;
use Botble\Ecommerce\Models\DuitkuReference;
...
class OrderSupportServiceProvider extends ServiceProvider
{
     ...
     */
    // Proses Pembayaran Di Mulai 1
    public function processPostCheckoutOrder($products, $request, $token, $sessionCheckoutData, $response)
    {
        // untuk melihat paymentMethod
        // dd($request->all());

        // dd($products, $request, $token, $sessionCheckoutData, $response);
        $groupedProducts = $this->cartGroupByStore($products);
        // dd($groupedProducts);
        $currentUserId = 0;
        ...
        // Proses Pembayaran
        $paymentData = $this->processPaymentMethodPostCheckout($request, $totalAmount, $sessionCheckoutData);
        // Proses Pembayaran
        if($paymentData['type'] == 'bank_transfer')
        {
            if ($paymentData['result_transaksi_duitku']['statusMessage'] == 'error') {
                return $response
                    ->setError()
                    ->setNextUrl(route('public.checkout.information', $token))
                    ->setMessage($paymentData['result_transaksi_duitku']['message'] ?: __('Checkout error!'));
            } else {
                $post_order_id = $paymentData['order_id'];
                for ($i=0; $i < count($post_order_id); $i++) {
                    $duitku_reference = new DuitkuReference;
                    $duitku_reference->order_id = $post_order_id[$i];
                    $duitku_reference->reference = $paymentData['result_transaksi_duitku']['reference'];
                    $duitku_reference->save();
                }
                return redirect()->away($paymentData['result_transaksi_duitku']['paymentUrl']);
     ...
     * @return array|mixed
     */
    public function processPaymentMethodPostCheckout($request, $totalAmount, $sessionCheckoutData)
    {
        // dd($sessionCheckoutData);
        $status_digital = '';
        if(array_key_exists('created_order_address_id', $sessionCheckoutData))
        {
            $status_digital = 'tidak';
        } else {
            $status_digital = 'ya';
        }
        $paymentData = [
            ...
            'charge_id' => null,
            'zip_code' => $sessionCheckoutData['zip_code'],
            'status_digital' => $status_digital,
            'email' => $sessionCheckoutData['email'],
            'phone' => $sessionCheckoutData['phone'],
            'name' => $sessionCheckoutData['name'],
            'address' => $sessionCheckoutData['address'],
            'city' => $sessionCheckoutData['city'],
            'state' => $sessionCheckoutData['state'],
            'country' => $sessionCheckoutData['country'],
        ];

        return apply_filters(FILTER_ECOMMERCE_PROCESS_PAYMENT, $paymentData, $request, $sessionCheckoutData);
    }
     ...
     * @throws BindingResolutionException
     */
    public function processGetCheckoutSuccess($token)
    {
